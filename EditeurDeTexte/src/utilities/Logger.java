...
public class Logger {
	 ...
	 */
	private static final String FOOTER = "</ol>" + LINE_SEPARATOR + "</body>"
			+ LINE_SEPARATOR + "</html>";
	...
	private Logger() {
		...
		_FileName = "Logs" + File.separator + "Log_"
				+ formatter.format(new Date()) + ".html";
		try {
			_Logger = new FileWriter(_FileName, true);
			_Logger.write(getHeader());
			_Logger.flush();
		} catch (IOException e) {
			System.err
	...
	private String getHeader() {
		String header = "<!DOCTYPE HTML>" + LINE_SEPARATOR + "<html>"
				...
				+ _FileName + "</title>" + LINE_SEPARATOR + "</head>"
