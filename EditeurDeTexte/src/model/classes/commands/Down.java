...
public class Down implements ICommandVisitor {
	...
	public void visit(ISection aSection) {
		if (aSection.getTitle().hasCursor()
				&& aSection.getIntroduction().getLines().size() != 0) {
			aSection.getIntroduction().getLines().get(0)
					.setCursorLocation(aSection.getTitle().getCursorLocation());
			aSection.getTitle().removeCursor();
		} else if (aSection.getTitle().hasCursor()
				&& aSection.getIntroduction().getLines().size() == 0) {
			if (aSection.getParent() == null) {
				ISection section = Editor
						.getEditor()
						.getCurrentDocument()
						.getSection(
